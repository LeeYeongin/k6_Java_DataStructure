...
//			System.out.print("  (" + i + ", " + heap[i] + ") ");
			System.out.print("  (" + i + ", " + heap[i].src + " -> " + heap[i].dest + ", " + heap[i].weight + ") ");
		System.out.println();
	}
	...
	public Edge DeleteMin() {
		...
		int i, j;
		Edge x = heap[1]; // root
		Edge last = heap[n]; // 마지막 리프 노트값
		n--;

		for (i = 1, j = 2; j <= n;) {
			if (j < n) // j==n이면 오른쪽 leaf가 없는 경우
				if (heap[j].weight < heap[j + 1].weight) // 오른쪽 leaf가 왼쪽 leaf보다 클 경우
					j++;
