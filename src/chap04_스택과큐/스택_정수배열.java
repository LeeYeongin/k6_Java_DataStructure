...
//--- 스택에 x를 푸시 ---//
	public void push(int x) throws OverflowIntStackException {
		if (isFull()) // 스택이 가득 참
			throw new OverflowIntStackException("push: can't push data");
		stk[ptr++] = x;
	}
...
//--- 스택에서 데이터를 팝(정상에 있는 데이터를 꺼냄) ---//
	public int pop() throws EmptyIntStackException {
		if (isEmpty()) // 스택이 빔
			throw new EmptyIntStackException("pop: nothing to pop");
		return stk[--ptr];
...
//--- 스택에서 데이터를 피크(peek, 정상에 있는 데이터를 들여다봄) ---//
	public int peek() throws EmptyIntStackException {
		// if(ptr<=0) 대신에 사용:
		if (isEmpty()) // 스택이 빔
			throw new EmptyIntStackException("peek: nothing to read");
...
//--- 스택을 비움 ---//
	...
	public void clear() throws EmptyIntStackException {
		 ...
		 */
		if (isEmpty()) // 스택이 빔
			throw new EmptyIntStackException("clear: stack is already emtpy");
		ptr=0;
...
//--- 스택 안의 모든 데이터를 바닥 → 정상 순서로 표시 ---//
	public void dump() throws EmptyIntStackException{
		if (isEmpty())
			throw new EmptyIntStackException("dump: nothing to read");
		...
		while (true) {
			...
			case 5: //clear
				try {
					...
					System.out.println("스택이 이미 비어있습니다." + e.getMessage());
					e.printStackTrace();
				}
				break;
			}
