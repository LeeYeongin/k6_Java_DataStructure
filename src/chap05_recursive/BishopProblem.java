...
import chap05_recursive.Stack41.EmptyGenericStackException;
...
class Stack41 {
	...
	// --- 생성자(constructor) ---//
	public Stack41(int capacity) {
		data = new ArrayList<Point1>(capacity);
...
public class BishopProblem {
	public static void EightQueen(int[][] d) throws EmptyGenericStackException {
		...
		Stack41 st = new Stack41(100); // 100개를 저장할 수 있는 스택을 만들고
		Point1 p = new Point1(ix, iy);// 현 위치를 객체로 만들고
		...
		while (true) { // 교제 175페이지 코드에 주석으로 알고리즘 로직을 작성
			...
			if (iy >= 0) { // 체스판의 다음 행에 퀸을 배치할 수 있으면
				p = new Point1(ix, iy); // 가능한 다음행 위치를 객체로 생성
				st.push(p); // 스택에 현 위치 객체를 push
				d[ix][iy] = 1;
				count++;
				iy++;
			...
			if (iy < 0) { // 다음 행에 퀸을 배치할 위치가 없거나 8개를 모두 배치하였다면 pop()
