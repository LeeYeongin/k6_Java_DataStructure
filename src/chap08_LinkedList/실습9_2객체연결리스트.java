...
class LinkedList2 {
	...
	//전달된 element를 찾을 때 comparator 객체를 사용한다 (이름으로 비교할지 번호로 비교할지 결정)
	{
	...
	//임의 값을 삽입할 때 리스트가 오름차순으로 정렬이 되도록 한다
	{
		...
		while(p != null) {
			if(cc.compare(p.data, element) < 0) {
				...
				if(q == null) { // 새로운 node를 처음에 넣을 경우
					newNode.link = p;
					first = newNode;
					return;
				} else { // 새로운 node를 중간에 넣을 경우
					q.link = newNode;
					newNode.link = p;
					return;
				}
		...
		q.link = newNode; // 새로운 node를 마지막에 넣을 경우
	}
	...
	public static void main(String[] args) {
		...
		do {
			...
			case Add :                          
				...
				data.scanData("입력", 3); // 왜 3을 전달? -> 둘다 입력 , 0을 주면 아무것도 입력하지 않음
