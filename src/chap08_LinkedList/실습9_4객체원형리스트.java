...
class CircularList {
	...
	public CircularList() { //head node -> 빈 노드를 하나 생성
	...
	public void Add(SimpleObject3 element, Comparator<SimpleObject3> noOrder) // 임의 값을 삽입할 때 리스트가 오름차순으로 정렬이 되도록 한다
	{
		Node3 newNode = new Node3(element);
		Node3 p = first.link, q = first;
		
		while(p != first) {
			if(noOrder.compare(p.data, element) < 0) {
				q = p;
				p = p.link;
			} else {
				q.link = newNode;
		...
		do {
			...
			case Delete: // 
				...
				if(num < 0)
					...
					System.out.println(num + "데이터 삭제에 성공했습니다.");
				break;
			case Show: 
				l.Show();
				break;
